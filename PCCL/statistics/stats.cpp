...
void Stats_thd::combine(Stats_thd *stats)
{
...
#if CHECK_CONFILICT
    valid_txn_cnt += stats->valid_txn_cnt;
    invalid_txn_cnt += stats->invalid_txn_cnt;
    #if ABORT_BATCH
    re_execute_txn_cnt += stats->re_execute_txn_cnt;
    #endif
#endif
    ...
    // Transaction stats
    txn_total_time_span += stats->txn_total_time_span;
...
void Stats::print(bool prog)
{
    ...
    if (totals->total_runtime > 0)
    {
        tput = totals->txn_cnt / (totals->total_runtime / BILLION);
#if ISEOV && CHECK_CONFILICT
        valid_tput = totals->valid_txn_cnt / (totals->total_runtime / BILLION);
#endif
        c_tput = totals->cross_shard_txn_cnt / (totals->total_runtime / BILLION);
    ...
    fprintf(outf, "tput         =%f\ttxn_cnt=%ld\n", tput, totals->txn_cnt);
#if ISEOV && CHECK_CONFILICT
    fprintf(outf, "valid_tput         =%f\tvalid_txn_cnt=%ld\tinvalid_txn_cnt=%ld\tvalid_percentage=%f\n", valid_tput, totals->valid_txn_cnt, totals->invalid_txn_cnt, (double)totals->valid_txn_cnt/totals->txn_cnt);
    fprintf(outf, "NodeLatency=%f\n", (totals->txn_total_time_span / BILLION) / totals->txn_cnt);
    #if ABORT_BATCH
    fprintf(outf, "re_execute_txn_cnt=%ld\n", totals->re_execute_txn_cnt);
    #endif
#endif    
...
void Stats_thd::print(FILE *outf, bool prog)
{
    ...
    double valid_tput = 0;
    if (total_runtime > 0){
        valid_tput = valid_txn_cnt / (total_runtime / BILLION);
    }
    ...
    file.close();
#if ISEOV && CHECK_CONFILICT
    fprintf(outf, "\ntput=%f\ntxn_cnt=%ld\nvalid_tput=%f\nvalid_txn_cnt=%ld\ninvalid_txn_cnt=%ld\nvalid_percentage==%f\n", tput, txn_cnt, valid_tput, valid_txn_cnt, invalid_txn_cnt, (double)valid_txn_cnt/valid_tput);
#else
    fprintf(outf, "\ntput=%f\ntxn_cnt=%ld\n", tput, txn_cnt);
#endif
